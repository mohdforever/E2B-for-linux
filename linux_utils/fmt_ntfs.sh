...
#fi
 ...
 label="EASY2BOOT"
 sudo /sbin/mkfs.ntfs -F -f -L "$label" $device
 ptn=${device: -1}
...
sleep 5
 ...
 sudo $DIR/bootlace.com --floppy=1 --ntfs $device
...
echo ""
echo "Use   sudo ./udefrag -om $device  to defrag this volume."
echo "You will need to unmount the volume first and udfefrag is a 32-bit program (see www.easy2boot.xyz for instructions)."
exit 1
